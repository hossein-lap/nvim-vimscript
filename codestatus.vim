...
hi User1 ctermbg=4 ctermfg=232 guibg=#005a8e guifg=#ffffff "cterm=bold gui=bold
hi User2 ctermbg=4 ctermfg=232 guibg=#005a8e guifg=#ffffff cterm=bold gui=bold

" current config section {{{
setlocal laststatus=2                                    " show status line
setlocal statusline=                                     " status line config
setlocal statusline+=%1*%{StatuslineGitBranch()}         " git brunch in current directory
setlocal statusline+=\ \                                 " The current mode
setlocal statusline+=--                                  " The current mode
setlocal statusline+=%{toupper(g:currentmode[mode()])}   " The current mode
setlocal statusline+=--                                  " The current mode
setlocal statusline+=\ \                                 " The current mode
"setlocal statusline+=%2*\ \»                             " blank space
setlocal statusline+=%1*%f                               " Full path
setlocal statusline+=%1*\                                " blank space
setlocal statusline+=%2*%m                               " blank space
setlocal statusline+=%1*\                                " blank space
setlocal statusline+=%1*%r                               " blank space
setlocal statusline+=%=                                  " Switch to the right side
setlocal statusline+=%1*\ \                              " blank space
setlocal statusline+=%1*Ln\ %l,                          " blank space
setlocal statusline+=%1*\ Col\ %c                        " cursor current position
setlocal statusline+=%1*\ \                              " blank space
setlocal statusline+=%1*%{''.(&fenc!=''?&fenc:&enc).''}  " Encoding
setlocal statusline+=%1*\ \                              " blank space
setlocal statusline+=%1*%Y                               " file type
"setlocal statusline+=%1*\ \«                             " blank space
setlocal statusline+=\                                   " blank space
"setlocal statusline+=\                                   " blank space
" }}}
